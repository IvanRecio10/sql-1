...
-- Ejercicio 8 OPCION 2: Contar cuántos usuarios con edades diferentes saben el lenguaje 'Java'.
    ...
    SELECT count(*) FROM usuarios_lenguajes WHERE lenguaje = 'Java' Group by edad
...
SELECT COUNT(*) AS total FROM usuarios_lenguajes
WHERE edad > 28;
...
SELECT lenguaje, COUNT(*) AS total FROM usuarios_lenguajes
WHERE edad < 30
GROUP BY lenguaje
ORDER BY total DESC
LIMIT 1;
...
SELECT nombre, COUNT(lenguaje) AS total_lenguajes FROM usuarios_lenguajes
GROUP BY nombre;
...
SELECT COUNT(DISTINCT lenguaje) AS total_lenguajes FROM usuarios_lenguajes
WHERE edad > 25;
